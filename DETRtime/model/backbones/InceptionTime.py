...
class Inception(ConvNet):
    ...
    Daniel F. Schmidt, Jonathan Weber, Geoffrey I. Webb, Lhassane Idoumghar, Pierre-Alain Muller,
    François Petitjean
    """
    ...
    def __init__(
        ...
        """
        logging.info(f"Inception max pool {maxpools}")
        self.bottleneck_size = bottleneck_size
        # these are the 4 concatenated parallel convolutions, width of the inner tensor passed
        # through the network
    ...
    def _module(self, depth):
        ...
        In parallel it uses a simple convolution with kernel size 1 with max pooling for stability
        during training. The outputs of each convolution are concatenated, followed by batch
...
class Inception_module(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # logging.info(f"Inception time input {x.size()}")
