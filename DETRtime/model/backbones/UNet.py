...
class Block(nn.Module):
    def __init__(
        self, in_filters, out_filters, kernel_size=5, stride=1, padding=2, activation=nn.ReLU()
    ):
...
class Encoder(nn.Module):
    def __init__(
        self, output_size=1024, kernel_size=5, filters=(129, 128, 256, 512), pools=(4, 3, 2)
    ):
        super(Encoder, self).__init__()
        assert (len(filters) - 1) == len(pools)
        self.encBlocks = nn.ModuleList(
            [Block(filters[i], filters[i + 1], kernel_size) for i in range(len(filters) - 1)]
        )
...
class Decoder(nn.Module):
    ...
    def forward(self, x, encblocks):
        ...
        for i in range(len(self.decBlocks)):
            ...
            x = self.upconv[i](x)
            # should reshape exactly such that after concatenation we again have doubling of
            # everything
...
class UNet(nn.Module):
    def __init__(
        ...
        self.encoder = Encoder(
            filters=self.encChannels, output_size=bottleneck, pools=pools, kernel_size=kernel_size
        )
        self.decoder = Decoder(filters=self.decChannels, pools=pools[::-1], kernel_size=kernel_size)

        self.output = nn.Sequential(nn.Conv1d(self.decChannels[-1], nb_filters, 1), nn.Sigmoid())
