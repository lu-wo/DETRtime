...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses):
        ...
        if self.num_classes == 3:  # hardcoded class weights
            ...
            # empty_weight = torch.tensor([0.61, 0.83, 6.59, self.eos_coef]) #calculated, but found
            # to be worse
            # empty_weight = torch.tensor([0.61, 1.4, 6.59, self.eos_coef]) #adjusted saccades
            # higher movie
            ...
            # empty_weight = torch.tensor([0.5, 3.6, 1.7, 6.31, 4.0, self.eos_coef])#sleep,labels,
            # best so far
            empty_weight = torch.tensor([0.5, 4.0, 1.7, 6.31, 4.0, self.eos_coef])
            # logging.info("Weighted by events")
            # empty_weight = torch.tensor([0.94, 0.55, 0.53, 1.29, 1.95, self.eos_coef])#adjusted to
            # events
            logging.info(f"Weights: {empty_weight}")
    ...
    def loss_cardinality(self, outputs, targets, indices, num_boxes):
        ...
        boxes
        This is not really a loss, it is intended for logging purposes only. It doesn't propagate
        gradients
        """
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        loss targets dicts must contain the key "boxes" containing a tensor of dim [nb_target_boxes,
        2]
        ...
        loss_giou = 1 - torch.diag(
            generalized_time_iou(box_cxw_to_xlxh(src_boxes), box_cxw_to_xlxh(target_boxes))
        )
    ...
    def forward(self, outputs, targets):
        ...
        Parameters:
             ...
             targets: list of dicts, such that len(targets) == batch_size.
                      The expected keys in each dict depends on the losses applied, see each loss'
                      doc
        """
