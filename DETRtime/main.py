...
def get_args_parser():
    ...
    # * Backbone
    parser.add_argument(
        "--backbone", default="pcnn", type=str, help="Name of the convolutional backbone to use"
    )
    ...
    parser.add_argument(
        "--back_channels", default=16, type=int, help="Defines the number of embedding channels"
    )
    parser.add_argument(
        "--back_layers", default=12, type=int, help="Defines the number of layers in the backbone"
    )
    parser.add_argument(
        "--maxpools", nargs="+", type=int, help="Optionally define maxpools for ConvNet classes"
    )
    # * Transformer
    parser.add_argument(
    ...
    parser.add_argument(
        "--set_cost_bbox", default=10, type=float, help="L1 box coefficient in the matching cost"
    )
    parser.add_argument(
    ...
    parser.add_argument("--num_classes", default=3, type=int, help="number of classes to predict")
    ...
    parser.add_argument(
        "--output_dir", default="", required=True, help="path where to save, empty for no saving"
    )
...
def main(args):
    ...
    logging.basicConfig(filename=f"{args.output_dir}/{time.time()}info.log", level=logging.INFO)
    ...
    if args.resume:
        logging.info("Resume training with model.")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if test_stats["loss"] < min_val_loss:
            logging.info("New minimum validation loss. Saving as checkpoint_best_val.pth !")
            min_val_loss = test_stats["loss"]
            checkpoint_path = output_dir / "checkpoint_best_val.pth"
            torch.save(
                {
                    "model": model.state_dict(),
                    "optimizer": optimizer.state_dict(),
