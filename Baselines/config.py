...
config["data_dir"] = (
    "/itet-stor/wolflu/net_scratch/projects/ICML_Seg_Baselines/"
    + "EEGEyeNet_experimental/data/segmentation/"
    + "ICML_movie_min_segmentation_minseq_500_margin_1_amp_thresh_10000"
)
config["dice_weights"] = [0.866, 0.106, 0.027]  # label distribution
config["dice_weights"] = (
    np.array(config["dice_weights"]) / np.array(config["dice_weights"]).sum()
)  # inverse the weights for the dice loss
...
def build_file_name():
    all_EEG_file = f"{config['task']}_with_{config['dataset']}"
    all_EEG_file = f"{all_EEG_file}_synchronised_{config['preprocessing']}"
    return all_EEG_file
