...
class Ensemble:
    def __init__(self, path, model_name, nb_models, loss, batch_size=64, **model_params):
        """
        This class provides an interface to create an ensemble of models, one can implement a
        tensorflow/keras version of Ensemble_torch and use the training pipeline
        :param path: path to the folder where the models are stored
        :param model_name: name of the model to load
        :param nb_models: number of models to create as ensemble, if nb_models == 1 then we create a
        single model
