...
def collate_fn(batch, num_classes=3):
    ...
    target = torch.nn.functional.one_hot(
        torch.from_numpy(np.array(target)), num_classes=num_classes
    )
    return [data, target]
...
class Ensemble_torch:
    ...
    (majority decision in case of a classifier).
    """

    def __init__(
        self, path, model_name="CNN", nb_models=5, loss="dice-loss", batch_size=64, **model_params
    ):
        """
        ...
        self.load_file_pattern = re.compile(f"{model_name[:3]}.*.nb.*pth", re.IGNORECASE)
    ...
    def fit(self, train_dataloader, validation_dataloader):
        ...
        for i in range(self.nb_models):
            logging.info(":---------------------------------------")
            logging.info(f"Start fitting model number {i + 1}/{self.nb_models} ...")
            ...
            logging.info(":---------------------------------------")
            logging.info(f"Finished fitting model number {i + 1}/{self.nb_models} ...")
    ...
    def predict(self, test_dataloader):
        ...
        for model in self.models:
            ...
            logging.info("FInished test set prediction.")
    ...
    def save(self, path):
        ...
        for i, model in enumerate(self.models):
            ckpt_dir = f"{path}{self.model_name}_nb_{i}.pth"
