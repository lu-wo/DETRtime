...
class BaseNet(nn.Module):
    ...
    def __init__(
        ...
        if loss == "dice-loss":
            ...
            self.loss_fn = DiceLoss(weight=torch.tensor(dice_weights), p=2, reduction="mean")
    ...
    def save(self):
        ...
        """
        ckpt_dir = f"{self.path}{self.model_name}_nb_{self.model_number}.pth"
        torch.save(self.state_dict(), ckpt_dir)
        logging.info("Saved new best model to ckpt_dir")
