...
class EEGNet(BaseNet):
    ...
    In our implementation it is built on BaseNet and can therefore use the same interface for
        training as models based on ConvNet. We only define the layers we need, the forward pass, and a
        method that returns the number of hidden units before the output layer, which is accessed by
        BaseNet to create the same output layer as for ConvNet models.
    """
    ...
    def __init__(
        ...
        logging.info(f"Number of trainable parameters: {n_params_trainable}")
