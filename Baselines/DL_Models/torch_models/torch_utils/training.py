...
def train_loop(dataloader, model, loss_fn, optimizer):
    """
    Performs one epoch of training the model through the dataset stored in dataloader, predicting
    one batch at a time.
    Using the given loss_fn and optimizer.
    Returns training metrics (loss for regressions, loss and accuracy for classification) of the
    epoch.
    ...
    for batch, (X, y) in enumerate(dataloader):
        ...
        y_dice = (
            make_one_hot(y_dice, 5) if config["dataset"] == "sleep" else make_one_hot(y_dice, 3)
        )
        if torch.cuda.is_available():
            y_dice = y_dice.cuda()
...
def validation_loop(dataloader, model, loss_fn):
    ...
    val_scores = classification_report(
        ground_truth, predictions, target_names=TARGET_NAMES, digits=3
    )
...
def test_loop(dataloader, model):
    ...
    train_scores = classification_report(
        correct_pred, predictions, target_names=TARGET_NAMES, digits=3
    )
