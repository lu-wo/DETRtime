...
class PyramidalCNN(ConvNet):
    """
    The Classifier_PyramidalCNN is one of the simplest classifiers. It implements the class ConvNet,
    which is made of modules with a specific depth, where for each depth the number of filters is
    increased.
    """
    ...
    def __init__(
        ...
        self.nb_features = (
            depth * nb_filters
        )  # For pyramidal we increase the nbfilters each depth layer
        super().__init__(
    ...
    def _module(self, depth):
        ...
        activation. Finally, MaxPooling is also used. The number of filters / output channels is 
        increases with the depth of the model.
        Padding=same
        """
        return nn.Sequential(
            Pad_Conv(kernel_size=self.kernel_size),
            nn.Conv1d(
                in_channels=self.nb_channels if depth == 0 else depth * self.nb_filters,
